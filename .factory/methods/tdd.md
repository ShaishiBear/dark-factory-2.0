...
- **Lint-clean test files are part of the acceptance contract.** The acceptance tests are frozen after RED, so a test file that fails the repository's static checks can never be repaired later; the kernel checks the files you wrote before it commits them and hands a failure back to you once.
