...
`behaviors` is a list; each item carries its own `id`. A behavior whose `then` pins behaviour the issue says is kept (it holds on the unchanged tree and must still hold after the change) may carry `"kind": "guard"` beside its `id`; the independent test author then proves it with a checkpoint that must pass before and after the change instead of one that must fail first. Omit `kind` for an ordinary behavior; do not use a guard for the behaviour the issue asks to change. `invariants`, `out_of_scope`, `risks` and `ambiguities` are string arrays (plain strings, not objects). Do not invent requirements. Put any genuinely unresolved product decision in `ambiguities`; the deterministic compiler will stop rather than guess. Every requested behavior must map to a testable seam.
