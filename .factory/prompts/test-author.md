...
Write `$ARTIFACTS_DIR/test-spec.json` as version `2.0` with `checkpoints`, one per AC exactly once. Each checkpoint is `{ "acceptance_id": "AC-N", "cwd": "repo/relative/cwd", "argv": ["executable", "arg"], "files": ["repo/relative/test-file"], "expected_failure": "stable output fragment" }`. Every declared file must be test-shaped: under `tests/` or `__tests__/`, or named `*.test.*`, `*.spec.*`, `test_*` or `conftest.py`; both the commit authority and the RED gate refuse anything else.
