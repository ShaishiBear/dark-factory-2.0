...
| `factory_kernel/capabilities.py`, `effect_broker.py` | Deterministic grants for the six fixed effects (exact subject, derived resources, role, source and policy digests, lease bundle or the recorded legacy serial route, one use, expiry, revocation epoch); an in-process broker that serves `merge_exact_head` and `observe`: re-observes the PR, consumes the grant once in a durable journal, rechecks stop and fence immediately before the one remote call, spends through the existing `merge_squash`, records the observation; timeouts stay uncertain and never replay | `runtime._merge_and_verify` (both merge entry points) derives the grant from `merge-authorization.json` and merges through the broker; `merge-grant.json` and `effect-journal.jsonl` land beside the run's artifacts | the separate-identity broker process and its OS-boundary tests; the other four operations (their call sites still spend directly); any TCB reduction |
