...
**Every clock in the ladder comes from one record, and every wrapper bounds what it contains.** `harness/budget.py` reads `harness/budgets.json` and gives each scope `ceil(p100 × headroom / 60) × 60` seconds: the static, unit, e2e, holdout and mutation rungs; the application and factory mutation families and the sub-bounds inside them; the whole `harness/ci.py` ladder; `scripts/factory_evidence.py`; `scripts/factory_evidence_spine.py`; `harness/post_merge.py`; the kernel's quick-gate and evidence-spine stages; and the `timeout-minutes` of `dark-factory-ci.yml` and `dark-factory-main-regression.yml`. Each scope also declares what runs inside it, and `harness/budget.py` refuses a record in which a wrapper's budget is less than the sum of the budgets it contains plus a stated margin — so raising an inner budget past its wrapper stops the gate in the change that raises it, rather than arriving as a timeout on a validation run (D-075). The mutation rung is the ladder's most expensive step and the only one whose cost grows with the catalogue: nine application defects, each re-running all four channels on a mutated tree, and 406 factory trust-root defects, each in its own copy of the trust root. The rung reports its own clock as it goes: `MUTATION_TIMING id=<id> seconds=<n> outcome=<...>` per application defect, `seconds=<n>` on every factory defect line, `MUTATIONS_SECONDS` / `FACTORY_MUTATIONS_SECONDS` totals, `MUTATIONS_OK defects=<n> seconds=<n> budget=<n>`, and a `MUTATIONS_BUDGET_WARNING` while it is still green. Every timed rung of the ladder prints `RUNG_SLOW` on the same threshold, and a rung that times out keeps whatever it had already printed (D-073).
...
| `factory_kernel/validation_meter.py`, `provider_gateway.py`, `billing_reconciliation.py` | Spend classes and one ledger reservation per validation bundle; a route-fixed, credential-stripping gateway contract with an injected transport; per-invocation receipt reconciliation that fails closed | `execution_probe.ProbeRunner` meters diagnostic launches; `scripts/factory_test_author_probe.py` opens one bundle | the metered lane while the credential is withheld |
| `factory_kernel/gateway_server.py` | The gateway contract on a loopback socket: one channel per preregistered route, a shared channel token for the process under validation, the credential held only in the gateway's closure, upstream redirects refused, responses buffered, meter records and a summary written beside the app log; a recording-only ledger that holds no allowance | `harness/appproc.py` `HttpApp` when `FACTORY_VALIDATION_GATEWAY=1` and `.factory/kernel.json` carries `validation.gateway` (it carries none: the launcher refuses to enable the gateway) | the owner's policy block; egress containment of the process under validation; streaming pass-through; the separate-identity broker |
...
| `factory_kernel/capabilities.py`, `effect_broker.py` | Deterministic grants for the six fixed effects (exact subject, derived resources, role, source and policy digests, lease bundle or the recorded legacy serial route, one use, expiry, revocation epoch); an in-process broker that serves `merge_exact_head`, `publish_candidate` and `observe`: re-observes the PR (or re-derives the candidate's head, tree, branch and clean status from the worktree), consumes the grant once in a durable journal, rechecks stop and fence immediately before the one remote effect, spends through the existing `merge_squash` or `push_branch` + `create_pr`, re-observes the result; timeouts stay uncertain and never replay | `runtime._merge_and_verify` (both merge entry points) derives the merge grant from `merge-authorization.json`; `runtime._publish_build` (inline and the `publish` step) derives the publication grant from the worktree and the run's publication artifacts; `merge-grant.json`, `publish-grant.json` and `effect-journal.jsonl` land beside the run's artifacts | the separate-identity broker process and its OS-boundary tests; the other three operations (their call sites, the re-head's forced push and the safe-revert PR still spend directly); any TCB reduction |
| `factory_kernel/experiments.py` | The experiment registry: four reviewed families as data (runner id, metrics, work bound, claim scope, limitations). `lookup-workload-v1` (the existing data-only probe) and `repository-boundary-v1` (deterministic import, layer, cycle and affected-test counts over the session's frozen repository context for each candidate's declared touched paths; no code runs; files outside the context are counted as unanalysed) are runnable; `public-contract-probe-v1` and `migration-rehearsal-v1` are registered schemas that refuse to run and say why | `Exploration.experiment` validates, budgets, binds targets and runs every experiment through the registry; the reasoner's context carries the registry | the two sandboxed families (need the WP06 worker view and a disposable database), `experiment_runner.py` (leases, sandboxes), `research.py` |
| `factory_kernel/predictions.py` | Forecasts frozen before results: each candidate's per-criterion prediction in force for the round (the round's reasoned assessment, else the registration, inapplicable when the repository context changed), with its exact subject, measurement contract (the criterion's unit), data cutoff, source and horizon, marked uncalibrated; actuals matched by subject and contract into `within` / `outside` / `not-comparable` / `inapplicable` / `unknown` | `Exploration.experiment` records `prediction_outcomes` on every observation beside the measurements and claim observations | calibration analysis over many sessions (WP11), predictions for factory-level decisions outside Preflight |
| `factory_kernel/outcome_router.py` | What an authenticated factory outcome is evidence of, from structure only: the kernel's refusal reason code and its authority, the recorded handoff, the rules registered before selection and the independent findings; exactly one of implementation defect, strategy contradiction, environment/provider failure, evidence gap, policy change or unknown, with what it licenses (invalidated ancestry and reopening only for a contradiction; repair within the frozen acceptance only for a defect); the refusal's prose is never read | `StrategyRejection.assess` records the classification on every strategy assessment; `after_import` classifies an outcome with no registered rules from the refusal code alone | performance, contract and migration predicates; automatic reconsideration consuming an existing delegation; the coordinator wakeup |
| `factory_kernel/graph_transport.py` | The project graph over HTTP: `GET /api/project-graph` returns the bounded snapshot (or, with `after_version=<n>`, the exact delta against the projection of the events that existed at that version, or an explicit resnapshot instruction when the cursor is ahead of the project or further behind than the bound), named by a snapshot identity over its sources; `GET /api/project-graph/details?id=<opaque>` returns one node with its edges after authorization and before existence; owner-only, read-only, no job, no paid call, distinct event and observation cursors (the observation cursor stays empty until an observer supplies one) | `FrontDoorApplication` serves both routes; the only routes that accept a query string; `frontdoor_static/decision-graph.js` is their only client | requirement claims in the graph (they need a programme candidate the HTTP app does not hold), external observations feeding the observation cursor, pagination beyond the projection's own bounds |
| `factory_kernel/calibration.py`, `harness/experiments/learning_protocol.json` | Calibration of frozen forecasts against recorded outcomes, report-only: `join_outcomes` joins every recorded prediction outcome to the forecast that was in force when the observation was recorded (reconstructed from the events before it, so a later assessment or a tampered identity is `unjoined-forecast`, never scored), counts one trajectory once, keeps an unreported cost unknown and lists unmeasured candidates and failed experiments; `calibration_report` groups by project, task family, method version and model (the ledger records no model name: `unrecorded`), gives exact interval coverage and absolute error, Brier scores for probability forecasts (none are recorded, and the report says so), unknown-outcome rates and cost missingness, splits into the protocol's preregistered cohorts (`learning_protocol.json`: projects and sessions, no overlap; the evaluation cohort is empty by declaration until a held-out project exists), flags a trajectory that appears in two cohorts as contaminated, and exposes `admission_observations` in the shape `lessons.gate_results` reads; `python -m factory_kernel.calibration --store --repository --project --protocol` prints it | the lesson-admission evaluator decides on the report's coverage observations under its protected policy; the CLI over a real intent store | any admission, ranking or benefit claim; retrieval-on versus retrieval-off comparisons (no experience packets exist); a model group (no model name is recorded); time-separated cohorts (the ledger's `created_at` is kept on every row but the protocol separates by project and session) |
| `factory_kernel/experience.py` | Experience packets, advisory only: `retrieve(role, task, approved_scope, budget)` returns an `ExperiencePacket` of admitted lessons bound to their proposals by digest, retrieval-eligible for the role under the installed protected lesson policy, applicable to the task (same contamination group or a declared mechanism family), bounded by count and bytes, each with provenance, `not_established` and lineage as counter-evidence; the allowlist is plan/investigate/context/implement/repair and every other role (the holdouts, certifiers, acceptance-test author, conformance) gets an empty `blind` packet before the record source is read; `no_memory_packet` records the fresh challenger's empty context; `lesson_records` reads this run's `investigation-*.json` admissions (the only retained lesson records) | `KernelRuntime._experience_context`, the one funnel both `_agent` paths pass through: refuses a blind role whose context carries the packet marker, retrieves for permitted roles, records `experience-<role>.json` beside every stage by reference only (the artifacts directory is readable by the run's tool-bearing roles, including the blind `test_author` and `conformance`, so no lesson text is written there), appends the packet text only when it has items | any persistent lesson store across runs, an installed lesson policy (none exists, so every production packet is empty), retrieval for `test_author` or `conformance`, the retrieval-on versus retrieval-off comparison, sandbox file delivery of packets (packets travel in the prompt only) |
| `factory_kernel/maintenance.py`, `.factory/maintenance-policy.json` | Governed self-maintenance, proposals only: `classify_change(paths, declared_effects, declared_tier, facts)` derives the tier deterministically as the maximum of the path-derived tier (the security guard's protected set, pinned by test) and the policy's tier for every path-derived or declared effect (judge, floor, capability, evidence, policy, prompt, deploy, security, product); a declared lower tier is recorded as a downgrade attempt and ignored; supplied facts can only add refusals (a lowered or dropped floor, a role gaining an operation or a new role, a removed detector, suppressed evidence) and facts the classifier needs but lacks make the classification `unknown`; activation is refused unless the lane is in the policy's activated autonomous lanes, which is empty. `propose_change(incident, objective, paths)` verifies the incident is an outcome classification or lesson admission record by its own digest (free text refused), bounds the objective and records a maintainer proposal with the actor roles (proposed by / established by / delivered by). `prepare_shadow` names the old authority as the judge of a candidate (never itself); `compare_shadow` stores both results and blocks on any disagreement; `request_cutover` is refused under the installed policy and names every missing gate. `python -m factory_kernel.maintenance classify|propose` prints the records | the CLI; nothing in the kernel's run path calls it yet | any lane activation, any patch generation, the maintenance workflow, the guard protecting `.factory/maintenance-policy.json` and `.factory/lesson-policy.json` (a FACTORY_RULES section 5 amendment through section 12, not done here), a self-maintenance demonstration |
| `factory_kernel/frontdoor_static/decision-graph.js`, `decision-graph.css` | The decision and implementation graph in the owner's Front Door page: five views (Intent, Explore, Prove, Built system, Reconsider) over `GET /api/project-graph`, an SVG with a keyboard-selectable node list alternative (a table with kind, label, state and why), a details panel from `GET /api/project-graph/details`, exact client-side delta merges against the snapshot the delta names with a resnapshot fallback that never mixes lineages, polling at 5 s while a run or programme item is active, 30 s while visible and idle, paused while hidden; a node is green only when current with no recorded gap, a prediction is never listed as built and is marked predicted where it appears, the Reconsider view walks changed assumptions to the decisions and claims that depend on them; every string reaches the page through `textContent`, there is no inline script, and the only requests are the two GET routes (selecting a node changes nothing; approval, publication and stop keep their own controls) | `index.html` loads it; `frontdoor.js` refreshes it after every snapshot | observation-fed animation (no observation source exists), a browser journey in the canonical ladder (`harness/frontdoor_journey.py` drives it with agent-browser locally only), any action from the graph, pagination of the projection |
| `factory_kernel/experiment_runner.py` | Contained execution of registered experiments: `execute_registered` acquires a lease for the session's experiment slot and the shared runner slot in a SQLite lease store beside the intent store (`experiments.sqlite`), spawns a fresh interpreter on the module's fixed `--child` entry with an allowlisted environment (only PATH, the platform's start-up variables, a temp directory and a home cross; nothing else is inherited, so no credential is), an empty temporary working directory and a wall clock (30 s), hands it one JSON object (the validated spec and the session's frozen context) on stdin and reads one back; the child resolves the family through the registry only; the parent re-verifies the receipt (the requested family, exactly the requested strategies, the family's metrics, the exact spec digest, the frozen context, the family's claim scope); a timeout is recorded as a timeout that proves only that the bounded workload did not finish within the stated limit, a child that fails to start or exits is a failed attempt, a malformed or forged receipt is refused; every attempt is recorded and the lease is released whatever happened | `Exploration.experiment` runs every registered experiment through it and records the attempt on the observation | an OS or container sandbox (the child can still read the filesystem; the attempt says so), parallel experiments (one per session and one runner slot at a time), a persistent lease across service restarts (the store persists, the slot is released per attempt), `cancel` of a running child (there is no running child outside `execute_registered`) |
...
A turn cap ends a mutation worker's loop, and the gates judge its draft; and the worker has the tools its policy names. Build run 34033360798 (issue #103) ended `test_author` at its 30-turn cap (`error_max_turns`, 289 s, $2.63) with `ChatArea.test.tsx` written and edited on disk, and the kernel failed the build on the CLI's exit code without the static gate, the commit authority or RED ever seeing the file; the same transcript shows the model calling Glob (and Bash) and being told `No such tool available`, though the policy granted Read, Glob, Grep, Write and Edit. Two causes, two changes (D-065). First, `--bare` sets `CLAUDE_CODE_SIMPLE=1`, and in that mode the pinned CLI registers only Read and Edit whatever `--tools` says; the provider now launches every worker with `--safe-mode --setting-sources ""` instead (the same isolation: no CLAUDE.md, skills, plugins, hooks, MCP servers or settings file; built-in tools and permission rules work as documented; both measured on 2.1.245), the three mutation prompts name the five tools in one sentence and say there is no shell and no test runner, and the read-scope preflight also asks for a Grep over the tree and a Glob for the probe files and prints `grep_denied_outside_scope=`, `glob_outside_scope=`, `tools=` and `tools_missing=` beside its old fields (the `Read` deny rules reach Grep and Glob on 2.1.245: a tree-wide Grep omits the trust-root line, a Grep into the trust root is refused by name, a Glob omits the path). Second, for `test_author`, `implement` and `repair` only, an `error_max_turns` envelope is returned by the provider marked `cap_reached` instead of raised; the stage record, its timing row and its `FACTORY_STAGE` line say `cap_reached=true` (`outcome=ok` there says the worker returned), and the kernel runs exactly the gates a returned worker gets on what is in the checkout: the scoped static gate with its formatter pass and its one hand-back, the commit authority's D-064 rules, then RED or GREEN. A checkout with no change is refused as `no_draft_at_cap`; a test author's dirty checkout without a usable `test-spec.json` (absent, malformed, or declaring a file not on disk) as `no_spec_at_cap`, naming the files it wrote; and when a gate refuses a capped draft the needs-human comment says the cap was reached. Every other role's cap, and every budget stop, is the failed stage it was. The caps do not change.

The real cap envelope reaches the gates, and whitespace never costs a stage. Build run 34047586142 (issue #103) drafted correctly on its first `test_author` run, failed the scoped static gate on one biome formatting difference, was handed back, and its second run ended at the turn cap - where the kernel raised `did not return a JSON result envelope` and failed the build. Two causes, two changes (D-068). First, a real `error_max_turns` result event carries no `result` key at all (measured on that run: `duration_api_ms duration_ms errors fast_mode_* is_error modelUsage num_turns permission_denials queued_turn_count session_id stop_reason subagent_stats subtype terminal_reason total_cost_usd type usage uuid`; a success event adds `result`, `api_error_status` and the `ttft_*` timings), and `unwrap_result_envelope`'s shape guard demanded one before D-065's cap branch could run, so the cap path was unreachable for the only payload it exists for. An envelope is now well-formed when it is a mapping with `is_error`; `result` is required only of a NON-error envelope, and a capped envelope's `content` is the empty string. Second, the static gate applies the repository's FORMATTER - `uv run ruff format` for backend files, `bun x biome format --write` for frontend files, on the worker's declared files only - and re-runs the checks before it hands anything back; only a finding that survives the formatter reaches a worker. What the formatter rewrote is measured by file content, recorded in the static-gate artifact as `formatted` and printed as `FACTORY_STATIC_FORMATTED role=<role> attempt=<n> files=<paths>`, so a reformat is evidence and never silent. Lint fixes (`--fix`, `--unsafe-fixes`, `biome check --write`) are never applied: formatting is whitespace and cannot change behaviour, a lint fix can. A formatter that is missing, times out or changes nothing leaves the first pass's verdict exactly as it was, and the commit authority's file-union rule (D-064) still governs what may be committed.

### Daily regression on `main`

`.github/workflows/dark-factory-main-regression.yml` runs the full canonical harness against current `main` once a day (03:41 UTC) and on manual dispatch, with the worker's pins, postgres service and disposable validation environment copied verbatim (`tests/factory/test_factory_workflow_hygiene.py` asserts they agree). It holds `contents: read` and `issues: write` only. Success prints `MAIN_REGRESSION_OK head=<sha>`. Failure files one `priority:high` / `type:bug` issue for ordinary triage, comments on an existing one instead of duplicating, and adds `factory:needs-human` on the second consecutive failure. It never merges and never applies any other `factory:*` label. It also carries the four preflight probes — the model route, the effort level, the thinking budget and the read scope, each the worker's step body byte for byte and each `continue-on-error` so a probe never decides this job's verdict — because three of them left the worker's per-dispatch path once their questions were answered and their evidence has to keep arriving from somewhere (D-070).
