...
The "what does not exist" column names each package's principal gaps; the complete list is each checkpoint document's
"What does NOT exist" section, which the column points to and does not replace.
...
| WP02 | Validation meter, provider gateway contract, billing reconciliation, metered probes; offline CLI compatibility spike with a daily hosted record step; loopback gateway transport behind the validation launcher (off: no policy block registered) | yes | #245, #250, #251, #252 | yes (transport off) | the daily spike record had not fired on 18 Sept (cron lateness recorded); no metered validation run since | allowance visibility to the owner; any paid allowance; the daily record's first hosted appearance; streaming pass-through in the gateway transport |
...
code via `tests/factory/blueprint_adapter.py` (a thin adapter, no fixture answers), recorded at 164c6c0 (main d6bc4bb plus
one docs commit) after PR #248. The factory suite's last full local run, on the checkpoint 22 working tree over 9225655,
was 2996 tests with 32 failures and 3 errors: the 32 failures and 2 of the errors are the `test_factory_bootstrap` and
