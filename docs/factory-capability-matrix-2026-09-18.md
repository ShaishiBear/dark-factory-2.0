...
| WP10 | WP10A: read-only bounded graph endpoint with exact deltas, resnapshot instructions and authorized details over the real Front Door HTTP application (`graph_transport.py`); the pure projection `project_graph.py` existed since #243. WP10B (the static SVG/list UI, the five views, observation-fed cursors) is not built | endpoint (PR pending); UI: no | yes | PR pending | no (unmerged) | no (no owner has read the graph from the hosted Front Door) |
