...
|---|---|---|---|---|---|---|
| WP00 | Read-only `plan` job before any paid work; diagnostics retained on every path; early diagnostics directory | yes | yes | #239, #241 | yes (`dark-factory-worker.yml`) | yes: scheduled run 35294669709 (2026-09-18 01:16 UTC) answered idle with no service, toolchain or paid call |
...
| WP03 | Transition phase machine, receipts journal, release decision, `transition-status` reader; the eight public `transition_release` vectors | yes (pure part) | yes | #247 | yes (nothing calls it) | no transition exists |
...
code via `tests/factory/blueprint_adapter.py` since #247 (`python docs/implementation-blueprint-2026-09-17/conformance/run.py
