...
| WP08 | Experiment registry (`experiments.py`): four families as data, two runnable (`lookup-workload-v1`, `repository-boundary-v1`, deterministic and offline), two registered-not-runnable; wired into `Exploration.experiment` and the reasoner's context. Predictions, research, the experiment runner (leases, sandboxes) and the sandboxed families are not built | registry and boundary family (PR pending) | yes | PR pending | no (unmerged) | no (no owner exploration session has run an experiment in the hosted lane) |
