...
| WP06 | `.factory/tcb.json` record and verifier, entered into the host's authority closure; `capabilities.py` grants and the in-process `effect_broker.py` on the real merge path (`merge_exact_head`, `observe`); no broker process, no worker view, no reduction claimed | record; grants and in-process broker (PR pending) | yes | #248; broker PR pending | record: yes (the host reads it from `main`); broker: no (unmerged) | no (no merge has run through the broker in the hosted lane) |
