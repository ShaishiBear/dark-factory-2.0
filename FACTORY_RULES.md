...
- **Prompts show the shape their validator accepts; the kernel supplies what a worker cannot compute.** Every prompt that describes a validated artifact carries the exact JSON skeleton the deterministic compiler accepts, and the kernel hands workers the data they have no shell to derive: the merge-base diff to reviewers and the conformance authority, the applicable architecture policy ID sets to the governor, the validated contract and its hash to post-contract workers. Only kernel-written text is rendered for `$ARTIFACTS_DIR`; the untrusted invocation context (issue body, repro record, review JSON) is appended verbatim (D-028).
