...
- **Worker draft deadline:** `DRAFT_DEADLINE_FRACTION` = 0.8 of the role's turn cap (`draft_deadline_turn(role)`: turn 24 of 30; 0.6 until the first MiniMax M3 build was ended at turn 18 after 17 in-scope reads, D-063) for `test_author`, `implement` and `repair`. The provider's stream reader counts turns as distinct assistant message ids, exactly as a killed process's `num_turns` is counted, and kills the process the moment a turn past the deadline begins with no Write/Edit tool_use seen; the stage is refused as `no_draft_by_turn` (`DraftDeadlineMissed`), recorded with `draft_deadline_missed`, `draft_deadline_turn`, `reads` and up to 40 paths read, printed as `FACTORY_STAGE ... draft_deadline_missed=true reads=N`, and quoted in the needs-human comment; it is not transient and is never retried. No other role has a deadline (D-057).
