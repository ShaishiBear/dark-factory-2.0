...
- **The gate formats before it hands back.** Before the kernel commits a worker's files it runs the repository's scoped static checks on them (`factory_kernel/static_gate.py`). When they fail it applies the FORMATTER only - `uv run ruff format` for backend files, `bun x biome format --write` for frontend files, on the worker's declared files - and re-checks; only a finding that survives the formatter is handed back to a worker, and a hand-back is still bounded at one. What the formatter rewrote is measured by file content, written to the static-gate artifact as `formatted` and printed as `FACTORY_STATIC_FORMATTED`, so a machine edit to a draft is always visible. Lint fixes (`--fix`, `--unsafe-fixes`, `biome check --write`) are never applied: formatting is whitespace and cannot change behaviour, a lint fix can, and a behavioural defect belongs to the author who can judge it. A formatter that is missing, times out or changes nothing leaves the first verdict exactly as it was. Build run 34047586142 spent a whole second `test_author` process, thirty turns and its entire budget on one biome formatting difference and produced nothing (D-068).
