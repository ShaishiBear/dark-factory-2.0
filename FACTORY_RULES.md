...
Every run first proves its prerequisites and refuses otherwise: Issues enabled, `main` protected, every label the kernel can apply present (the eight `factory:*` controls plus `priority:*` and `type:*`, read from `factory_kernel.triage.label_vocabulary`), `OPENROUTER_API_KEY` and `SUPADATA_API_KEY` present, a raw request to `${ANTHROPIC_BASE_URL}/v1/messages` returning 200, and the pinned Claude Code CLI, launched as the kernel launches a worker, returning a non-error result from every configured model (`FACTORY_PREFLIGHT_MODEL_ROUTE_OK`). The first canary dispatch showed a probe that proves a different request than the worker makes proves nothing (D-010).
