...
- **Maintainer-lane PRs** are merged or armed for GitHub auto-merge by the `unattended-merge` job, only after the trusted verdict passes in the `human-maintenance` lane and the stop decision is clear. The job exports that combined decision and rechecks remote stop state after the job boundary. With no checkout or candidate code, it runs `gh pr merge --auto --squash --match-head-commit <judged head>`. GitHub CLI merges an immediately eligible PR or arms auto-merge while requirements are pending. Both paths retain the exact judged head and all branch protections; no administrator bypass is used. A changed head, unreadable stop state or refused merge fails closed. A later push re-runs the workflow against the new head.
