...
- Workers run as fresh Claude Code CLI processes with `--safe-mode` and no settings file (`--setting-sources ""`; `--bare` until D-065, whose simple mode registered only Read and Edit whatever the tool list said), an empty strict MCP configuration, slash commands disabled, a filtered environment, and an explicit tool list (`factory_kernel/worker_policy.py`). **No worker ever has Bash or Git.** They load no plugins, hooks or skills; the engineering methods a role follows (minimal complexity, deep-module design, vertical-slice implementation, red-loop diagnosis, the two review axes) are pinned text in `.factory/methods/` that the kernel injects into that role's prompt (`factory_kernel/methods.py`, manifest validated fail-closed).
- `plan`, `investigate`, `contract`, `context`, `architecture`, `test_author`, `implement`, `review`, `repair`, `conformance` get `Read`, `Glob`, `Grep`, `Write`, `Edit`. Only `test_author`, `implement` and `repair` are permitted to leave the worktree dirty; every other role's run is rejected if it changed anything. Every tool-bearing role also runs inside a **read scope** (`ROLE_PATH_SCOPE` in `factory_kernel/worker_policy.py`, D-057): the provider renders it as the CLI's `Read(...)`/`Edit(...)` permission rules instead of bare tool names, so a worker may read the product tree (`app/`, `docs/`, `README.md`, `CLAUDE.md`, `MISSION.md`, `FACTORY_RULES.md`) and its run's artifacts directory, may write `app/`, `docs/`, `README.md` and the artifacts (the three mutation roles) or only the artifacts (every other role), and is denied the trust root (`factory_kernel/`, `harness/`, `scripts/`, `tests/factory/`, `.github/`, the protected `.factory/` files) and `.git` itself, which is where the provenance and carry notes refs live (D-071). The one exception is `.factory/architecture.json`, which the architecture governor, the conformance authority and the standards reviewer are told to read. The `_agent_stage` funnel refuses a mutation-role request that carries no scope, `tests/factory/test_factory_read_scope_and_draft_deadline.py` asserts the rendered argv rule by rule on every gate, and a probe proves on the runner that the pinned CLI enforces the rules (`FACTORY_PREFLIGHT_READ_SCOPE_PROBE`, `scripts/factory_read_scope_probe.py`), daily on `main` and on a worker dispatch that sets `run_answered_probes` (D-070). Prompts name run outputs as `$ARTIFACTS_DIR/<file>` by contract; the kernel renders that placeholder to the absolute artifacts path before launch (`factory_kernel/prompt_render.py`), refuses any placeholder it cannot render, and names a worker that wrote to a literal `$ARTIFACTS_DIR` directory as that failure (D-026).
- `triage`, `holdout`, `architecture-holdout` and the three certifiers get **no tools**.
- **Prompts show the shape their validator accepts; the kernel supplies what a worker cannot compute.** Every prompt that describes a validated artifact carries the exact JSON skeleton the deterministic compiler accepts, and the kernel hands workers the data they have no shell to derive: the merge-base diff to reviewers and the conformance authority, the applicable architecture policy ID sets to the governor, the validated contract and its hash to post-contract workers. Only kernel-written text is rendered for `$ARTIFACTS_DIR`; the untrusted invocation context (issue body, repro record, review JSON) is appended verbatim (D-028). Every validator accepts each entry of a worker-written list as either a plain string or an object carrying that list's canonical key (`path`, `name`, `id`, `text`), normalises to the string before validating or hashing, and ignores a top-level `notes` field; prompts show plain strings and name `notes` as the place for explanations (`scripts/factory_shapes.py`, D-029).
- **The gate formats before it hands back.** Before the kernel commits a worker's files it runs the repository's scoped static checks on them (`factory_kernel/static_gate.py`). When they fail it applies the FORMATTER only - `uv run ruff format` for backend files, `bun x biome format --write` for frontend files, on the worker's declared files - and re-checks; only a finding that survives the formatter is handed back to a worker, and a hand-back is still bounded at one, as the RED gate's own hand-back is (section 3 step 5, D-069). What the formatter rewrote is measured by file content, written to the static-gate artifact as `formatted` and printed as `FACTORY_STATIC_FORMATTED`, so a machine edit to a draft is always visible. Lint fixes (`--fix`, `--unsafe-fixes`, `biome check --write`) are never applied: formatting is whitespace and cannot change behaviour, a lint fix can, and a behavioural defect belongs to the author who can judge it. A formatter that is missing, times out or changes nothing leaves the first verdict exactly as it was. Build run 34047586142 spent a whole second `test_author` process, thirty turns and its entire budget on one biome formatting difference and produced nothing (D-068).

- **Every worker is a bounded loop.** `ROLE_MAX_TURNS` in `factory_kernel/worker_policy.py` gives each role a turn cap that the kernel passes to the CLI as `--max-turns`; the CLI returns one JSON result envelope and a worker that runs out of budget or ends in an API error is a failed stage, not a result (`factory_kernel/providers.py`). An envelope is well-formed when it is a mapping carrying `is_error`; `result` is required only of a non-error envelope, because the real error envelope has no such key and requiring it made the cap path below unreachable for the payload it exists for (D-068). For a drafting role or a judge a turn cap is the same failed stage. For `test_author`, `implement` and `repair` **a cap ends the loop; the gates judge the draft**: the provider returns the result marked `cap_reached`, the stage record, its timing row and its `FACTORY_STAGE` line say `cap_reached=true`, and the kernel runs exactly the gates a returned worker gets (the scoped static gate with its formatter pass and its one hand-back, the commit authority's envelope rules, RED or GREEN) on what is in the checkout; a checkout with no change is refused as `no_draft_at_cap`, a test author's dirty checkout without a usable `test-spec.json` as `no_spec_at_cap` naming the files it wrote, and a gate that refuses a capped draft says in the needs-human comment that the cap was reached (D-065). The caps themselves do not change. Before this cap the only backstop was the 20-minute subprocess timeout. Every request also names an effort level (`ROLE_EFFORT`, passed as `--effort`: workers `medium`, judges `high`), because a turn cap and a dollar cap bound neither how long one turn thinks (D-055). A repository-mutation worker also has a **draft deadline**, which records rather than decides: if no Write/Edit tool call has appeared when a turn past `ceil(cap × DRAFT_DEADLINE_FRACTION)` (0.8: turn 24 of 30) begins, the provider notes it and keeps reading, the stage runs on to its turn cap or its wall, and its record, its timing row and its `FACTORY_STAGE` line carry `draft_deadline_missed=true reads=N` with the paths it read; the cap path above is then what judges whatever draft it left. The prompt states the turn (`$DRAFT_DEADLINE_TURN`, rendered from the policy) as advice. Until D-066 the provider killed the process there and the stage was refused as `no_draft_by_turn`, which truncated workers that would have drafted at turn 25-28 inside their own budget (D-057, D-063, D-066). Post-contract workers (`context`, `architecture`, `test_author`) receive the validated contract and the original issue in their prompt, hash first, so they do not rediscover the task from disk; prompts tell workers to search before reading whole files rather than to read the constitution end to end. Per-stage telemetry and wall time are written to the run's transcripts (`stage-timings.jsonl`, `agent-<role>.json`) and uploaded by the worker workflow for observability only (D-020).
...
**0. The carry.** Before the first model stage the kernel looks for this issue's **carry**: the
certified upstream of a previous build of the same issue, stored as a Git note on
`refs/notes/dark-factory-carry` and keyed by issue number. It is written by the kernel in exactly
one place — after step 4's architecture gate has returned `proceed`, never before — and it holds
`issue.json`, `issue-frontier.json`, the ticket and frontier, the raw and compiled contract,
context and design, the governor artifacts, and a bug's repro records, each with its own sha256.
It is reused only when the carry is for this issue, its `base_sha` is the commit this build is
cutting from, the issue's title and body still hash to what it recorded, the kernel commit that
wrote it is an ancestor of the one running now, the whole trust root still digests to the same
`policy_sha256`, and every artifact verifies; each failure prints `FACTORY_CARRY_MISS issue=#N
reason=<reason>` and the build runs in full. On a hit (`FACTORY_CARRY_HIT issue=#N base=… stages=…
age=…`) **only the four model stages of steps 1-4 are skipped**: the artifacts are restored and
`factory_protocol.py contract`, `factory_protocol.py context`, `factory_architecture.py compile`,
the governor decision check and `factory_architecture.py scope` all re-run over them exactly as
below, `issue.json` and `issue-frontier.json` stay the kernel's own fresh snapshot, and every
compiled artifact is compared byte for byte against the carried copy afterwards. Any refusal or
any drift discards the carry and runs the full build; nothing in the carry path can fail a build.
Nothing downstream of step 4 is ever carried, and a merge drops the issue's carry. A worker can
neither read nor write it: it has no Bash and no Git, and `.git` is denied to every tool-bearing
role (D-071).

1. **`plan`** (features) or **`investigate`** (bugs, decided by a `bug` / `type:bug` label). For bugs the worker also writes exactly one of two records. `repro.json` (a test-runner command and a named symptom) when an existing command already fails on the unchanged tree: the kernel **executes it** (`factory_kernel/repro.py`) and the run continues only if it fails and its output contains the symptom. `repro-deferred.json` (why no existing command can fail, the seam, and the exact symptom the acceptance tests will print) when the bug is a wrong behaviour no test covers, which is the common case: the kernel validates it, hands it to the contract worker as a deferred red loop, and after `factory_proof.py red` refuses to continue unless at least one RED checkpoint's recorded output contains that symptom (`verify_deferred_in_red`, `KernelRuntime._close_deferred_repro`). The red loop is mandatory in both shapes; only where it is observed differs. Both records present, or neither, escalates. `repro-observed.json` records the mode and, for the deferred shape, which checkpoint showed the symptom. Three bounds hold around that model-authored command. Its argv must start with one of the repository's test runners (`pytest`, `python -m pytest`, `uv run pytest`, `uv run python -m pytest`, `bun test`, `bun run test`, `bunx vitest run`) and every later argument is refused if it is an eval/exec flag, contains a shell metacharacter, is an absolute path or escapes the checkout; `python -c` and `npx` are not shapes. Its environment is built from an allowlist (`REPRO_ENV_KEYS`: PATH, HOME, temp dirs, locale, PYTHONPATH), so no GitHub, provider or validation credential the worker holds reaches it. And `_observe_repro` compares `git status --porcelain --untracked-files=all` before and after: any change to the worktree escalates before the contract worker reads it. A repro that passes, does not show the symptom, has a refused shape or dirties the tree escalates: a bug that cannot be made to go red cannot be contracted. This is a shape allowlist, not a sandbox: a selected test file can still run arbitrary code; the bounds limit what it can reach and leave behind.
2. **`contract`** → `scripts/factory_protocol.py contract` compiles and hashes the execution contract. Ambiguity fails closed; every acceptance criterion must be stated so an independent test can prove it. The compiler accepts `behaviors` either as a list of `{id, given, when, then, seam}` objects or keyed by AC id, and normalises the keyed spelling to the list before hashing, so downstream consumers see one form.
3. **`context`** (fed the contract hash) → `scripts/factory_protocol.py context` compiles the design: the exact set of `planned_files` the implementation may touch. The same step compiles the ticket and frontier (`scripts/factory_artifacts.py ticket`) from `issue-frontier.json`, the snapshot of the issue and every `Blocked by: #N` issue that the kernel fetched with its own GitHub authority before any model stage; the compiler holds no credentials and never calls GitHub.
...
5. **`test_author`** writes acceptance tests only. For a bug whose repro was deferred to RED, the kernel hands the test author the exact symptom string the RED gate will require; and the kernel hands the conformance worker (step 8) the policy-ID sets computed from the changed files, the basis its compiler judges (D-030). The kernel commits them through the **acceptance-test envelope**: the dirty set must equal exactly the files the test spec declares, and every one must be test-shaped (under `tests/` or `__tests__/`, or named `*.test.*`, `*.spec.*`, `test_*`, `conftest.py`). `scripts/factory_proof.py red` then replays them: one independently runnable checkpoint per acceptance criterion, covering every criterion exactly once. A checkpoint is `kind: red` (the default, and the only kind in a version 2.0 spec): it must fail on the unchanged tree with its declared `expected_failure` in the output, and pass at every GREEN. A version 2.1 spec may also declare `kind: guard` for a criterion that pins kept behaviour (a contract behaviour of `kind: guard`, or one whose Then says the behaviour is kept and must stay green): a guard has no `expected_failure`, must exit 0 at RED (`guard failed on the unchanged tree` otherwise) and at every GREEN (`guard broken by the implementation` otherwise), and a spec of only guards is refused because it would prove no change (D-058). The proof, the `RED_PROVED`/`GREEN_PROVED` lines and the evidence bundle count guards separately (`guards=N`). **A refused RED is handed back to its author once** (`KernelRuntime._red_gate`, `RED_HANDBACK_ATTEMPTS = 1`), on the same principle as the static gate's hand-back below: the kernel undoes the test-author commit (`git reset --mixed` back to the head that stage started from, so the draft is uncommitted and editable again), re-runs `test_author` once with its original context plus the refusal - the acceptance id, argv, cwd, exit code, seconds, declared `expected_failure`, reason and output tail from `red-proof-failure.json` (D-056) - and then re-runs the whole gate from scratch; a second refusal ends the build exactly as before. The brief says the two corrections that are open (make the declared string a fragment of what the command actually printed, or change the test's own query so it prints what was declared) and that every rule above still holds; a re-draft that drops a checkpoint or re-declares a `red` one as a `guard` is refused deterministically as `red_handback_weakened_spec`, naming the change, before the gate is re-run. A `guard` checkpoint's refusal, a launch or timeout `fault`, and a refusal that wrote no record at all are not declaration errors and are never handed back. Both attempts survive: `red-gate.log`/`red-gate.2.log`, `red-proof-failure.json`/`red-proof-failure.2.json` and the per-run stage records, and the needs-human comment quotes both. Build run 34054922788 lost a $15, forty-five-minute build to one mis-declared `expected_failure` with the correct test already on disk (D-069). The files are hashed and become **immutable for the rest of the build**. Runner output stored in evidence is sanitised of terminal control sequences before it is recorded (`factory_kernel/attached.py`), and every attach program verifies that its block survives the PR body round trip before reporting success.
...
7. **`review-spec`** and **`review-standards`** (two fresh processes with disjoint prompts and disjoint artifacts, so one judgement cannot colour the other). `factory_kernel/review.py` aggregates deterministically: a missing, malformed or mislabelled axis artifact, or a verdict that disagrees with its own findings, escalates; either axis failing fails the review. If it fails, exactly one **`repair`** pass runs under the same design envelope, followed by another GREEN replay and a second two-axis review that must pass. There is no second repair.
...
4. **Provenance pack verifies.** `scripts/factory_provenance.py fetch` retrieves the builder's published contract, context, design, governor verdict, proofs, impact and conformance artifacts, and `verify_pack` binds them to the expected head, base and issue. This is the only step that reads builder-produced artifacts, and it reads them in order to verify them, not to believe them. It runs before the code holdout because the holdout's proof summary is cross-checked against the note-bound RED proof (D-048).
5. **Blinded code holdout passes.** A model with no tools, running in an empty temporary directory outside the source checkout, is shown only the contract, changed-file list, diff and proof summary, and must return `pass`. The proof summary carries what the kernel has already proved deterministically: per-checkpoint RED outcomes (failing exit, expected failure, output excerpt; for a `guard` checkpoint a passing exit and no expected failure, labelled as a guard so kept behaviour is not judged unverified) at the RED commit, the GREEN replay at the head, the immutable acceptance files with their hashes, and base-versus-head test-definition counts for every test file the diff touches.
...
8. **Evidence spine closes.** `scripts/factory_evidence_spine.py` runs the core Evidence Bundle v5 (`scripts/factory_evidence.py`): trust-root touch/drift check, contract revalidation, security result, independent RED reconstruction and replay, GREEN replay, architecture policy/bindings recomputed, and the **full canonical harness** (`python harness/ci.py`: static, unit, real browser E2E, protected holdouts, application mutations) reaching `GATE_OK mode=full`. It then re-observes factory trust-root mutations and the immunity registry, re-verifies the provenance pack, loads the certificates, and closes all claims in `.factory/evidence-spine.json`. Ratchet floors in `.factory/locks/floor.json` must hold, and the head must not have moved.
   ...
   **Every clock in the ladder is a measurement, not a literal — and a wrapper bounds what it contains.** Every rung deadline, every mutation-family deadline, the whole-ladder deadline and the wrappers above it (`scripts/factory_evidence.py`, `scripts/factory_evidence_spine.py`, `harness/post_merge.py`, `harness/observe.py`, the kernel's evidence-spine stage and `dark-factory-main-regression.yml`'s `timeout-minutes`) are derived from `harness/budgets.json` (`harness/budget.py`: `ceil(p100(recorded totals for the scope) × headroom / 60) × 60`, headroom at least 1.5). Every scope also declares the scopes whose clocks run inside it, plus the contained work that carries its own bound and the measured work that carries none; `harness/budget.py` **refuses** a record in which any wrapper's budget is below that sum, so raising an inner budget past its wrapper stops the gate in the change that raises it instead of arriving as a `TimeoutExpired` on the validation run that mattered (D-075). Raising a budget therefore means recording the run that justifies it *and* raising every wrapper above it, in the same reviewed change; `tests/factory/test_factory_mutation_budget.py` refuses a record whose entries do not say where their numbers came from, and `tests/factory/test_factory_ladder_budget.py` refuses a wrapper that no longer contains its parts or a file that has gone back to a literal. The runners print a per-defect `MUTATION_TIMING id=<id> seconds=<n> outcome=caught|escaped` line, a family total (`MUTATIONS_SECONDS`, `FACTORY_MUTATIONS_SECONDS`), the total against the budget (`MUTATIONS_OK defects=<n> seconds=<n> budget=<n>`), and a `MUTATIONS_BUDGET_WARNING` once a run crosses `warn_fraction` of its budget **while it still passes**; every other rung of the ladder says `RUNG_SLOW step=<name> seconds=<n> timeout=<n>` on the same threshold, and a rung that does time out now keeps its partial output. No defect is ever skipped for being late: the clock bounds the rung, it never selects what runs. The factory family evaluates its copies concurrently and stops each copy's focused suite at its first red file, in the baseline's own cheapest-first order — the same verdict, since a suite is red as soon as one file is and a green suite still runs every file (D-073).
...
2. The locked fixture (`fixture_video_id`, `question_template` in `harness/harness.config.json`) is configured and the template names `{title}`; the frontend port is resolved.
3. Sign in with the validation account from the environment (`DARK_FACTORY_E2E_EMAIL` / `PASSWORD`). Before any browser opens, the harness asks each boundary itself and prints the answer: the backend login (`E2E_LOGIN_PROBE`), the frontend proxy (`E2E_PROXY_PROBE`), the bootstrap marker in the app process log and the fixture in `/api/videos` (`E2E_BOOTSTRAP_SEEN`, `E2E_VIDEOS_PROBE`), the journey's question, rendered from `question_template` with the locked fixture's title exactly as the catalog lists it (`E2E_QUESTION`; a row without a title is refused, `E2E_FIXTURE_TITLE_MISSING`), and the streaming route with that question (`E2E_STREAM_PROBE`, which spends one of the account's daily messages and deletes its conversation). The stream must answer 200 with at least one token, no error payload and a `sources` event whose first entry is the locked video; a stream that never called a retrieval tool carries no sources event and is refused here with the cause named (`reason=no-sources-event`, D-062), not in the browser.
4. Land on `/`, screenshot `authenticated.png`.
5. Send the same rendered question; record the page every half second until the answer arrives (`E2E_STREAM_UI`, `stream-states.jsonl`). The "Stop response" state must have been seen at least once unless the answer arrived within one poll.
...
**The ratchet only goes up.** When `.factory/locks/floor.json` is in a PR's diff, the same guard compares every numeric floor at the base with the head (`ratchet_regressions`, run from the base-anchored `trust-root-authority`): a key may be added or raised, never lowered or removed, and invalid JSON at the head is a regression. This applies on both lanes; being human waives the protected-path veto and nothing else. A floor can therefore rise through the human lane and cannot fall without first deleting this check, which is itself a protected path with a mutation (`security-ratchet-regression-unchecked`) that fails the factory suite if it is removed.
...
- **Maintainer-lane PRs** are merged by the `unattended-merge` job only after the trusted verdict passes in the `human-maintenance` lane and the stop decision is clear. With no checkout or candidate code, the job waits for required checks, requires a nonempty all-passing inventory, then rechecks remote stop immediately before `gh pr merge --squash --match-head-commit <judged head>`. It never arms deferred native auto-merge, which could otherwise execute after a later stop. The wait is bounded at22 minutes within a25-minute job (the quick job is bounded at20 minutes). A changed head, missing/failed/skipped check, unreadable stop state, timeout or refused merge fails closed. All branch protections remain; no administrator bypass is used. A later push re-runs the workflow against the new head.
...
- **Stop button.** With a stop active, maintainer PRs are still judged but not merged by the helper; a human may still merge manually if the checks are green.
- **Failure.** A red required check blocks every merge of that head for everyone. The helper does not leave a future merge scheduled after it exits. Stop and merge are separate GitHub operations, so this narrows the observation-to-effect interval but does not claim an atomic cross-API transaction.
- **Conflicts.** GitHub runs no `pull_request` workflow on a PR that conflicts with `main`, so `quick-authority` would never report. `trust-root-authority` runs on `pull_request_target` regardless, and after the verdict it asks GitHub for the PR's `mergeable_state`: `dirty` makes the required check red with `TRUST_ROOT_REFUSED pr is not mergeable (conflicting with base); rebase`, and nothing is armed. The verdict on the diff stands; only mergeability turned the check red, and a push that resolves the conflict re-runs both workflows. `unknown` (GitHub has not computed it yet) is retried briefly and then tolerated, because an unknown state is not evidence of a conflict. The ruleset deliberately does not require branches to be up to date: that would serialise every unrelated PR behind each merge and re-run every gate on each advance of `main`.
...
4. Every RED-hashed acceptance test — red files and guard files alike, since the proof's `files` map is the union of both (D-058) — must be byte-identical at the new tip, or the re-head refuses.
5. A rebase rewrites the test-author commit, so the RED proof is **re-issued by replay, never re-bound by hand** (D-045): the kernel locates the rebased test-author commit by shape (first above the new base, the fixed `git_authority` subject), checks it out detached, and runs `scripts/factory_proof.py red` there from a spec reconstructed out of the pack's own checkpoints. The commit's parent diff is judged by the same three rules the commit authority applies to a dirty checkout (D-064), read from the **checkpoints** rather than from the `files` map: a guard file no red checkpoint declares must not appear in the diff, every changed file must be declared, and every red-checkpoint file must be changed. Each guard file is then verified where the map alone cannot say it: it must exist at the rebased commit and hash there to what the pack recorded, or the re-head refuses naming the file and both hashes (D-072). Every checkpoint must still fail for its declared reason; one that passes after the rebase, a history of the wrong shape, or a re-issued commit that is not an ancestor of the new tip refuses the re-head.
6. What a new head invalidates is then recomputed, in order: GREEN replay, the `conformance` worker and its compiler, the final GREEN, the quick gate, a clean-tree check.
7. The branch is pushed with `--force-with-lease=refs/heads/<branch>:<judged head>`, the one legitimate non-fast-forward push in the kernel: the rebase rewrote history by construction, and the lease makes the remote refuse if anything but the judged head is there.
8. The contract and final proof are re-attached, the provenance note is republished at the new head (`GitHubClient.push_branch`, `scripts/factory_protocol.py attach`, `scripts/factory_proof.py attach`, `scripts/factory_provenance.py publish`), a `<!-- dark-factory-rehead: {…} -->` marker is posted, and the PR goes back to `factory:needs-review`.
...
Every run first proves its prerequisites and refuses otherwise: Issues enabled, `main` protected, every label the kernel can apply present (the eight `factory:*` controls plus `priority:*` and `type:*`, read from `factory_kernel.triage.label_vocabulary`), `OPENROUTER_API_KEY` and `SUPADATA_API_KEY` present, GitHub Actions permitted to create pull requests (refused on an explicit `false`; reported as `FACTORY_PREFLIGHT_PR_PERMISSION_UNVERIFIED` when the token cannot read the setting, since `GITHUB_TOKEN` lacks `administration:read`; D-022), a raw request to `${ANTHROPIC_BASE_URL}/v1/messages` returning 200, and the pinned Claude Code CLI, launched as the kernel launches a worker, returning a non-error result from every configured model (`FACTORY_PREFLIGHT_MODEL_ROUTE_OK`). The first canary dispatch showed a probe that proves a different request than the worker makes proves nothing (D-010).
...
- **Worker timeout:** each role runs under its own wall, `stage_timeout_seconds(role)` = ceil(turn cap × 45 s/turn × 1.5) in `factory_kernel/worker_policy.py` (2025 s for a 30-turn role, 675 s for a 10-turn judge); `provider.timeout_seconds` = 2700 is the maximum every wall must fit under, not a wall itself. The CLI is read as a stream (`--output-format stream-json`), and `provider.idle_timeout_seconds` = 420 kills a process that prints no event for that long; a hang is retried once, a second hang and a wall timeout are terminal, and either leaves the turns, events and partial output it showed in the stage record (D-054). The dispatch job is capped at 360 minutes; merge and the unchanged post-merge ladder run in a separate 210-minute job under the same workflow concurrency lock. A same-run digest-bound handoff transports qualification, and merge rechecks current authorization before its fresh App effect. These are platform ceilings, not observed completion times.
- **Transient provider retries:** `provider.transient_retries` = 2 (bounded 0..3). A stage whose CLI process ends in an explicitly transient error (a dropped stream, `overloaded`, a rate limit, 429/502/503/504, `ECONNRESET`, `ETIMEDOUT`, `socket hang up`) is re-launched as a fresh process with the same prompt after a 5 s then 15 s backoff; before a retry of `test_author`, `implement` or `repair` the kernel restores the worktree to its pre-attempt state. A turn cap, a budget stop, a missing model or any other error is terminal and refused on the first occurrence. The dollar budget is per process, so a stage's effective ceiling is `max_budget_usd × (1 + transient_retries)`; telemetry records every attempt (`factory_kernel/providers.py`, D-031).
- **Worker turns:** per-role caps in `factory_kernel/worker_policy.py` `ROLE_MAX_TURNS` (triage 20; plan 30; investigate 30; contract 30; context 24; architecture 30; test_author 30; implement 30; review-spec 30; review-standards 30; repair 30; conformance 30; holdouts and certifiers 10). Every role's wall (its cap at the measured ceiling of 45 seconds per turn, with 1.5× headroom) must fit under the provider maximum (`assert_caps_fit_timeout`, tested); a cap the maximum would cut off first is a configuration error, because the CLI stopping at its cap returns a clean failed stage while a wall timeout is a kill (one that now records what the stream showed, D-054). Exceeding the cap fails the stage. Changing a cap is a trust-root change (D-025).
- **Worker budget:** per-role dollar backstops in `ROLE_MAX_BUDGET_USD`, passed as `--max-budget-usd` (builder roles 12; reviews 4; triage and blinded authorities 2). A budget-stopped worker is a failed stage. The CLI's reported cost for a non-Anthropic model is a fallback-priced figure until reconciled against the OpenRouter dashboard, so these are runaway stops, not budgets.
- **Worker effort:** per-role levels in `ROLE_EFFORT`, passed as `--effort` on every request from the pinned CLI's scale (`low`, `medium`, `high`, `xhigh`, `max`): every worker that edits or drafts against a checkout (`plan`, `investigate`, `contract`, `context`, `architecture`, `test_author`, `implement`, `repair`, `conformance`, both reviews) at `medium`, one notch below the CLI's default so thinking is bounded but not disabled; every judge (the five validation authorities and triage) at `high`, the default. `provider.effort_overrides` in `kernel.json` is the per-deployment `{role: level}` override, refused at load for an unknown role or level. A request without a level is refused by the `_agent_stage` funnel like one without a turn cap. Whether the route honours the level is measured daily on `main`, and on a worker dispatch that sets `run_answered_probes` (`FACTORY_PREFLIGHT_EFFORT_PROBE ... honoured=true|false`, `scripts/factory_effort_probe.py`, D-070), which prints and never refuses; each stage records the thinking its stream showed (`thinking_tokens`, `thinking=<n>` on the stage line) and the level it ran at. Changing a level is a trust-root change (D-055).
- **Worker read scope:** per-role `PathScope` rows in `ROLE_PATH_SCOPE` (`factory_kernel/worker_policy.py`), rendered by the provider as `Read(./<pattern>)` and `Edit(./<pattern>)` allow rules plus `Read(//<artifacts>/**)`/`Edit(//<artifacts>/**)` for the run's artifacts directory in `--allowedTools`, and `Read`/`Edit` deny rules for the trust root in `--disallowedTools`. The mutation roles (`test_author`, `implement`, `repair`) read the product tree and write `app/`, `docs/`, `README.md`; every drafting role reads the same and writes only artifacts; `architecture`, `conformance` and `review-standards` additionally read `.factory/architecture.json`; judges have no tools and an empty scope. A file inside the working directory is readable without any rule, so the deny list is the read boundary and the `Edit` allow list the write boundary; the rendered argv is asserted rule by rule by `tests/factory/test_factory_read_scope_and_draft_deadline.py` on every gate, and the probe verifies the denial on the live route daily on `main` (and on a worker dispatch that asks for it), refusing that run only when the trust-root file came back through a tool result. Changing a scope is a trust-root change (D-057).
- **Worker draft deadline (a recorded signal, not a limit):** `DRAFT_DEADLINE_FRACTION` = 0.8 of the role's turn cap (`draft_deadline_turn(role)`: turn 24 of 30; 0.6 until the first MiniMax M3 build was ended at turn 18 after 17 in-scope reads, D-063) for `test_author`, `implement` and `repair`. The provider's stream reader counts turns as distinct assistant message ids, exactly as a killed process's `num_turns` is counted, and notes the moment a turn past the deadline begins with no Write/Edit tool_use seen. Nothing is killed and nothing is refused for it: the stage runs on to its turn cap or its wall and is recorded, whatever its outcome, with `draft_deadline_missed`, `draft_deadline_turn`, `reads` and up to 40 paths read, printed as `FACTORY_STAGE ... draft_deadline_missed=true reads=N` and quoted in the needs-human comment of a build that ends closed. The turn cap ends the loop and the gates judge the draft (`no_draft_at_cap` / `no_spec_at_cap` when there is none), so the flag is read beside cost and latency, never as a verdict. Until D-066 the process was killed there and the stage refused as `no_draft_by_turn`. No other role has a deadline (D-057, D-063, D-066).
- **Worker thinking cap:** per-role rows in `ROLE_THINKING_CAP` (`factory_kernel/worker_policy.py`), every row `None` today, exported by the provider as `MAX_THINKING_TOKENS` in the CLI's environment only for a role whose cap is set (0 disables thinking; a budget is at least 1024 tokens, the smallest the CLI sends as given). `provider.thinking_cap_overrides` in `kernel.json` is the per-deployment `{role: cap}` override, refused at load for an unknown role or a cap the CLI would not honour as given. Whether the route honours the budget is measured daily on `main`, and on a worker dispatch that sets `run_answered_probes` (`FACTORY_PREFLIGHT_THINKING_CAP_PROBE ... honoured=true|false`, `scripts/factory_thinking_cap_probe.py`: uncapped, 1024 and 0 on the effort probe's prompt, D-070), which prints and never refuses; a row is set only from that line. Setting a row is a trust-root change (D-059).
