...
8. **Evidence spine closes.** `scripts/factory_evidence_spine.py` runs the core Evidence Bundle v5 (`scripts/factory_evidence.py`): trust-root touch/drift check, contract revalidation, security result, independent RED reconstruction and replay, GREEN replay, architecture policy/bindings recomputed, and the **full canonical harness** (`python harness/ci.py`: static, unit, real browser E2E, protected holdouts, application mutations) reaching `GATE_OK mode=full`. It then re-observes factory trust-root mutations and the immunity registry, re-verifies the provenance pack, loads the certificates, and closes all claims in `.factory/evidence-spine.json`. Ratchet floors in `.factory/locks/floor.json` must hold, and the head must not have moved.
   ...
   **The mutation rung's clock is a measurement, not a literal.** Both mutation families are bounded by a budget derived from `harness/mutations/budget.json` (`harness/mutation_budget.py`: `ceil(p100(recorded totals for the scope) × headroom / 60) × 60`, headroom at least 1.5), and every caller — `harness/ci.py`, the nested factory call and the spine's independent re-observation — reads that derivation instead of carrying a number. Raising a budget therefore means recording the run that justifies it in the same reviewed change, and `tests/factory/test_factory_mutation_budget.py` refuses a record whose entries do not say where their numbers came from. The runners print a per-defect `MUTATION_TIMING id=<id> seconds=<n> outcome=caught|escaped` line, a family total (`MUTATIONS_SECONDS`, `FACTORY_MUTATIONS_SECONDS`), the total against the budget (`MUTATIONS_OK defects=<n> seconds=<n> budget=<n>`), and a `MUTATIONS_BUDGET_WARNING` once a run crosses `warn_fraction` of its budget **while it still passes**; every other rung of the ladder says `RUNG_SLOW step=<name> seconds=<n> timeout=<n>` on the same threshold, and a rung that does time out now keeps its partial output. No defect is ever skipped for being late: the clock bounds the rung, it never selects what runs. The factory family evaluates its copies concurrently and stops each copy's focused suite at its first red file, in the baseline's own cheapest-first order — the same verdict, since a suite is red as soon as one file is and a green suite still runs every file (D-073).
