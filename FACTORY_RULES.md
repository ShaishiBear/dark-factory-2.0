...
5. **Blinded code holdout passes.** A model with no tools, running in an empty temporary directory outside the source checkout, is shown only the contract, changed-file list, diff and proof summary, and must return `pass`. The proof summary carries what the kernel has already proved deterministically: per-checkpoint RED outcomes (failing exit, expected failure, output excerpt) at the RED commit, the GREEN replay at the head, the immutable acceptance files with their hashes, and base-versus-head test-definition counts for every test file the diff touches.
