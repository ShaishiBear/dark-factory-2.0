...
- **Every worker is a bounded loop.** `ROLE_MAX_TURNS` in `factory_kernel/worker_policy.py` gives each role a turn cap that the kernel passes to the CLI as `--max-turns`; the CLI returns one JSON result envelope and a worker that hits its cap, runs out of budget or ends in an API error is a failed stage, not a result (`factory_kernel/providers.py`). Before this cap the only backstop was the 20-minute subprocess timeout. Post-contract workers (`context`, `architecture`, `test_author`) receive the validated contract and the original issue in their prompt, hash first, so they do not rediscover the task from disk; prompts tell workers to search before reading whole files rather than to read the constitution end to end. Per-stage telemetry and wall time are written to the run's transcripts (`stage-timings.jsonl`, `agent-<role>.json`) and uploaded by the worker workflow for observability only (D-020).
