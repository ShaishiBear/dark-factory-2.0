...
- **Worker turns:** per-role caps in `factory_kernel/worker_policy.py` `ROLE_MAX_TURNS` (triage 20; plan 30; investigate 30; contract 30; context 24; architecture 30; test_author 30; implement 30; review-spec 30; review-standards 30; repair 30; conformance 30; holdouts and certifiers 10). Every cap must fit under the provider timeout at the measured ceiling of 35 seconds per turn (`assert_caps_fit_timeout`, tested); a cap the timeout would cut off first is a configuration error, because a timeout records nothing while the CLI stopping at its cap returns a measured failed stage. Exceeding the cap fails the stage. Changing a cap is a trust-root change (D-025).
